    ...
    return <ul className={styles.container}>
        {this.state.fruitList.map(item => {
            return <li className={styles.item} key={item.id}>
                ...
                <p>商品名：{item.name}</p>
                <p>￥{item.price}</p>
                <Button onClick={() => this.props.add(item)} style={{backgroundColor:'rgb(103, 194, 58)',borderColor:'rgb(103, 194, 58)'}} type="primary">加入购物车</Button>
            </li>
        })}
    </ul>;
  }
...
/**
 * 
 * @param {*} dispatch 用于触发action
 */
const mapDispatchToProps = dispatch => {
  /**
   * 这个返回的对象中的方法，最终会映射给 GoodsList的props
   */
  return {
    add(goods){
